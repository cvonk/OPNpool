...
static datalink_a5_preamble_t preamble_a5 = { 0x00, 0xFF, 0xA5 };
static datalink_ic_preamble_t preamble_ic = { 0x10, 0x02 };
...
static void
_datalink_ic_head(datalink_ic_tx_head_t * const head, tx_buf_handle_t const txb, datalink_ctrl_typ_t const typ)
{
    head->ff = 0xFF;
    for (uint_least8_t ii = 0; ii < sizeof(preamble_ic); ii++) {
        head->preamble[ii] = preamble_ic[ii];
    }
...
static void
_datalink_ic_tail(datalink_ic_tx_tail_t * const tail, uint8_t const * const start, uint8_t const * const stop)
{
    uint8_t crcVal = 0;
...
static void
_datalink_a5_head(datalink_a5_tx_head_t * const head, tx_buf_handle_t const txb, datalink_ctrl_typ_t const typ, size_t const data_len)
{
    head->ff = 0xFF;
    for (uint_least8_t ii = 0; ii < sizeof(preamble_a5); ii++) {
        head->preamble[ii] = preamble_a5[ii];
    }
    head->hdr.ver = 0x01;
    head->hdr.dst = datalink_devaddr(DATALINK_ADDRGROUP_CTRL, 0);
...
static void
_datalink_a5_tail(datalink_a5_tx_tail_t * const tail, uint8_t const * const start, uint8_t const * const stop)
{
    uint16_t crcVal = 0;
    for (uint8_t const * byte = start; byte < stop; byte++) {
        crcVal += *byte;
...
datalink_tx_pkt(rs485_handle_t const rs485_handle, tx_buf_handle_t const txb, datalink_prot_t const prot, datalink_ctrl_typ_t const typ)
{
    ...
    switch (prot) {
        case DATALINK_PROT_IC: {
            ...
            _datalink_ic_head(head, txb, typ);
            ...
            _datalink_ic_tail(tail, crc_start, crc_stop);
            break;
        }
        case DATALINK_PROT_A5_CTRL:
        case DATALINK_PROT_A5_PUMP: {
            ...
            _datalink_a5_head(head, txb, typ, data_len);
            ...
            _datalink_a5_tail(tail, crc_start, crc_stop);
            break;
        }
    }
